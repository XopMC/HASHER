...
for algorithm in "${algorithms[@]}";do extra=();case "$algorithm" in hmac-*)extra=(-key benchmark-key);;pbkdf*|evpkdf*)extra=(-salt benchmark-salt -kiter 1);;esac
 for threads in 1 2 4 8;do times=();for((run=0;run<runs;++run));do start=$(perl -MTime::HiRes=time -e 'print time');"$exe" "-$algorithm" "${extra[@]}" -t "$threads" -i "$input" >/dev/null;end=$(perl -MTime::HiRes=time -e 'print time');times+=("$(perl -e 'print $ARGV[1]-$ARGV[0]' "$start" "$end")");done
 median=$(printf '%s\n' "${times[@]}"|sort -n|sed -n "$((runs/2+1))p");throughput=$(perl -e 'printf "%.0f",$ARGV[0]/$ARGV[1]' "$lines" "$median");printf '%s,%s,%.6f,%s\n' "$algorithm" "$threads" "$median" "$throughput" >> "$output";done;done
